...
async def check_conn(name, **kwargs):
...
async def main():
    ...
    await check_conn("Standard Default")

    # 2. With certifi
    try:
        import certifi
        await check_conn("With Certifi tlsCAFile", tlsCAFile=certifi.where())
    except ImportError:
    ...
    try:
        import certifi
        await check_conn("With Certifi + tls=True", tls=True, tlsCAFile=certifi.where())
    except ImportError:
        pass

    # 4. tlsAllowInvalidCertificates=True only
    await check_conn("tlsAllowInvalidCertificates=True only", tlsAllowInvalidCertificates=True)

    # 5. Without TLS parameters but tls=True
    await check_conn("tls=True only", tls=True)
    ...
    try:
        ...
        await check_conn("SSL Context CERT_NONE", ssl_context=ctx)
    except Exception as e:
